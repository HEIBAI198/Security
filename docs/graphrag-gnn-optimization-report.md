...
| 独立注册表审核正常包（npm 1770 + pypi 1782） | 3552 |
| hard negatives（全部可信来源） | 244 |
| 包节点 | 5952 |
| 异构事实边 | 44471 |
| 真实 depends_on 边 | 16583 |
| 归纳式训练边 | 14768 |
| train / val / test（时间外推 70/15/15） | 4166 / 892 / 894 |

数据质量状态：正样本占比 40.3%、PyPI 负样本占比 50.2%、负/正比 1.48、时间戳覆盖率 100%、正样本元数据覆盖率 41.6%、负样本 review_tier 可审计（explicit_curated 1054 / dependency_closure 2498）、held-out 演示包重叠 0、与全量恶意池重叠 0，审计零 warning，`--fail-on-warning` 通过。
...
环境：torch 2.7.0+cu128，CUDA 可用，NVIDIA GeForce RTX 4060 Laptop GPU。产物：`package_risk_graphsage.pt`、`package_embeddings.npy`、`package_embedding_index.json`、`graphsage_eval.json`。最佳轮次 12，温度 2.375，`decision_threshold=0.9`、`online_decision_threshold=0.9`，校准验证 `calibration.verified=true`（val ECE 0.0669）。

评估（graphsage_eval.json，阈值 0.9）：

| Split | Samples | Accuracy | Precision | Recall | F1 | ROC-AUC | FPR@0.9 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| train | 4166 | 0.5999 | 0.9333 | 0.0083 | 0.0165 | 0.9764 | 0.04% |
| val | 892 | 0.7422 | 1.0000 | 0.3611 | 0.5306 | 0.9498 | 0.0% |
| test | 894 | 0.7170 | 0.9820 | 0.3028 | 0.4628 | 0.8892 | 0.37% |

说明：

- 测试集是时间外推（未来恶意包），当前 7 个名字/关键词特征只能覆盖一部分；`recall_at_fpr_0.05` 在 test 上约 0.30、val 上约 0.54、train 上约 0.74，说明特征越新越难外推，这正是诚实评估要暴露的问题。
- 无邻居在线模式：良包高分率（>=0.9）0.65%（v2 曾高达 39%），恶意包高分率 11.7%（阈值保守），`benign_mean_score` 0.469、`malicious_mean_score` 0.732。
...
- 标签可信度：负样本与全量恶意池去重（重叠 0），`review_tier` 分层记录（explicit_curated / dependency_closure），演示包从训练数据剔除（held-out 重叠 0）。
...
- 校准验证：验证集 ECE 门禁（<=0.10），`calibration.verified` 写入元数据，未验证时 `score_kind` 降级为 similarity；单包详情支持传入项目子图。
- 降级：NumPy fallback 禁用，无 torch 环境返回 unavailable；PyG 是唯一可信后端。
- GraphRAG：embedding channel 已接入 `package_embeddings.npy`。
- 独立业务测试集：`scripts/gnn/evaluate_business_cases.py` + 真实 manifest 用例，frontend 案例 67 个包进入图模式，全部断言通过。
