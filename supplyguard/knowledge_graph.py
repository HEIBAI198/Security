...
def assign_node_positions(nodes: dict[str, GraphNode]) -> None:
    ...
    # Layered horizontal layout: columns spread across attack chain, tighter spacing
    col_x = 0
    for rank in sorted(buckets):
        bucket = buckets[rank]
        sorted_nodes = sorted(bucket, key=lambda item: (-item.score, item.label))
        for index, node in enumerate(sorted_nodes):
            if node.position:
                continue
            # Center nodes vertically; tighter row spacing (110px)
            total_h = len(sorted_nodes) * 110
            start_y = max(0, 300 - total_h // 2)
            node.position = {"x": col_x, "y": start_y + index * 110}
        col_x += 220  # Tighter column spacing
