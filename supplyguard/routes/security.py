...
def build_import_preflight_workspace_payload(
    *,
    workspace_override: dict[str, Any] | None = None,
    import_record: dict[str, Any],
) -> dict[str, Any]:
    """Create a clean workspace that represents only the project import preflight."""
    ...
    preflight = import_record.get("summary") if isinstance(import_record.get("summary"), dict) else {}
    file_stats = preflight.get("fileStats") if isinstance(preflight.get("fileStats"), dict) else {}
    dependency_files = preflight.get("dependencyFiles") if isinstance(preflight.get("dependencyFiles"), list) else []
    ci_files = preflight.get("ciFiles") if isinstance(preflight.get("ciFiles"), list) else []
    project_name = str(preflight.get("projectName") or import_record.get("projectName") or "当前项目")

    payload["summary"] = {
        "risk_score": 0,
        "risk_level": "preflight",
        "open_findings": 0,
        "critical_findings": 0,
        "repositories": 1,
        "dependencies": len(dependency_files),
        "build_steps": len(ci_files),
        "log_events": 0,
    ...
    payload["logs"] = []
    payload["graph"] = {
        "schema_version": "supplyguard.kg.v1",
    ...
    payload["facts"] = None
    payload["code_audit"] = None
    payload["dependency_audit"] = None
    payload["cicd_audit"] = None
    payload["artifact_trust"] = None
    payload["log_audit"] = None
    payload["multimodal_audit"] = None
    payload["evidence"] = []
    payload["normalized_findings"] = []
    payload["attack_paths"] = []
    payload["report"] = ""
    payload["report_html"] = ""
    payload["scanSuite"] = {"status": "preflight", "errors": []}
    payload["assistant"] = {
        "default_question": f"{project_name} 的预检结果说明了什么？",
        "answer": (
            f"{project_name} 已完成导入预检：发现 {int(file_stats.get('total') or 0)} 个文件，"
            f"{len(dependency_files)} 个依赖清单，{len(ci_files)} 个 CI/CD 配置。"
            "当前还没有执行完整扫描，因此综合风险评分和攻击路径需要点击开始扫描后生成。"
        ),
        "retrieval": [
            f"Project: {project_name}",
            f"Files: {int(file_stats.get('total') or 0)}",
            f"Dependency manifests: {len(dependency_files)}",
            f"CI files: {len(ci_files)}",
        ],
        "next_actions": ["开始扫描当前项目，生成代码审查、供应链、CI/CD 和图谱报告结果。"],
    }
    return payload
...
async def security_workspace_create(payload: WorkspaceCreateRequest) -> dict[str, Any]:
    ...
    base_payload = (
        build_import_preflight_workspace_payload(
