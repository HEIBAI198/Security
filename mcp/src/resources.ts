...
import {
  ...
  summarizeWorkspace,
  workspaceIdOf,
} from './supplyguard-client.js'

function jsonText(uri: string, payload: unknown) {
  return {
    contents: [
      {
        uri,
...
function markdownText(uri: string, text: string) {
  return {
    contents: [
      {
        uri,
...
export function registerResources(server: McpServer) {
  ...
  server.registerResource(
    'case-by-id',
    new ResourceTemplate('case://{caseId}', { list: undefined }),
    {
  ...
  server.registerResource(
    ...
    async (uri) => {
      ...
      if (workspaceId === 'latest') return markdownText(uri.href, workspace.report || '')
  ...
  server.registerResource(
    'evidence-by-id',
    new ResourceTemplate('evidence://{workspaceId}/{evidenceId}', { list: undefined }),
    {
  ...
  server.registerResource(
    'finding-by-id',
    new ResourceTemplate('finding://{workspaceId}/{findingId}', { list: undefined }),
    {
  ...
  server.registerResource(
    'attack-path-by-id',
    new ResourceTemplate('path://{workspaceId}/{pathId}', { list: undefined }),
    {
  ...
  server.registerResource(
    'dependency-by-name',
    new ResourceTemplate('dependency://{workspaceId}/{packageName}', { list: undefined }),
    {
