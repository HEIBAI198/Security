...
import {
  ArtifactTrustScanSchema,
  CicdScanSchema,
  CreateWorkspaceSchema,
  DependencyScanSchema,
  LogScanSchema,
  ReachabilityScanSchema,
  ReportSchema,
  RunTraceSchema,
  WorkspaceIdSchema,
  textResult,
...
function workspaceResult(workspace: Record<string, any>, module?: Parameters<typeof moduleStatusFromWorkspace>[1]) {
    ...
    async ({ workspaceId }) => {
      try {
        ...
        return textResult({
          status: 'success',
          workspaceId,
          summary: graph.graphSummary as Record<string, unknown>,
          evidenceGaps: evidenceGaps(workspace),
          nextActions: nextActions(workspace),
          raw: graph,
        })
